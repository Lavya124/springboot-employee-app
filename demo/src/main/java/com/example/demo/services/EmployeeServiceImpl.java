...
public class EmployeeServiceImpl implements EmployeeService{
    ...
    public Response<Employee> createEmployee(Employee employee) {
        ...
        for (Role role : employee.getRoles()) {
            Role existing = roleRepository.findByRoleName(role.getRoleName()).orElseGet(() -> roleRepository.save(role));
            roles.add(existing);
        }
        ...
        log.trace("Creating the employee data in database");

        try{
            Employee savedEmployee =  employee_repository.save(employee);
            ...
            response.setMessage("Internal server come while creating the employee account");
    ...
    public Response<Employee> getEmployeeById(Long id) {
        ...
        try{
            ...
            Optional<Employee>  getEmployeeById = employee_repository.findById(id);
    ...
    public Response<Employee> updateEmployeeById(Long id, Employee updatedEmployee) {
        Response<Employee> response = new Response<>();
        Optional<Employee> optionalEmployee = employee_repository.findById(id);
        ...
        try {
            if(optionalEmployee.isPresent()){
                ...
                Employee updatedEmploye = employee_repository.save(existingEmployee);
